...
var extensionReg = make(map[uint8]func() Extension)
...
// type (3, 4, or 5).
func RegisterExtension(typ uint8, f func() Extension) {
	switch typ {
...
func errExt(got uint8, wanted uint8) error {
...
func (r *RawExtension) ExtensionType() uint8 { return r.Type }
...
func (m *Reader) peekExtensionType() (uint8, error) {
	p, err := m.R.Peek(2)
	if err != nil {
		return 0, err
	}
...
func peekExtension(b []byte) (uint8, error) {
...
func (m *Reader) ReadExtension(e Extension) (err error) {
	...
	switch lead {
	case mfixext1:
		if p[1] != e.ExtensionType() {
			err = errExt(p[1], e.ExtensionType())
			return
		}
		p, err = m.R.Peek(3)
		if err != nil {
	...
	case mext8:
		...
		if p[2] != e.ExtensionType() {
			err = errExt(p[2], e.ExtensionType())
			return
		}
		read = int(p[1])
		off = 3
	...
	case mext16:
		...
		if p[3] != e.ExtensionType() {
			err = errExt(p[3], e.ExtensionType())
			return
		}
	...
	case mext32:
		...
		if p[5] != e.ExtensionType() {
			err = errExt(p[5], e.ExtensionType())
			return
		}
...
func AppendExtension(b []byte, e Extension) ([]byte, error) {
	...
	case 0:
		...
		o[n] = mext8
		o[n+1] = 0
		o[n+2] = e.ExtensionType()
		return o[:n+3], nil
	case 1:
		o, n = ensure(b, 3)
		o[n] = mfixext1
		o[n+1] = e.ExtensionType()
		n += 2
	case 2:
		o, n = ensure(b, 4)
		o[n] = mfixext2
	...
	default:
		...
		case l < math.MaxUint16:
			...
			o[n+3] = e.ExtensionType()
			n += 4
		default:
			o, n = ensure(b, l+6)
			o[n] = mext32
			big.PutUint32(o[n+1:], uint32(l))
			o[n+5] = e.ExtensionType()
			n += 6
		}
	}
