...
var (
	randLevel int64 = 0
	randCur         = rand.New(rand.NewSource(randLevel))
	randStack       = []*rand.Rand{}
)

func randPush() {
	randLevel++
	randStack = append(randStack, randCur)
	randCur = rand.New(rand.NewSource(randLevel))
}

func randPop() {
	randLevel--
	randCur = randStack[randLevel]
	randStack = randStack[0:randLevel]
}
...
func randIdent() string {
	...
	for i := range bts {
		bts[i] = idxChars[randCur.Intn(len(idxChars))]
	}

	// Use a `z` prefix so the randomly generated bytes can't conflict with
	// Go keywords (such as `int` and `var`).
	return "z" + string(idxChars[randLevel%idxCharsLen]) + string(bts)
}
