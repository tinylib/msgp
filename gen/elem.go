...
func (g *idgen) seed(seed ...string) {
	h := fnv.New64a()
	h.Write([]byte(strings.Join(seed, "/")))
	g.rand = rand.New(rand.NewSource(int64(h.Sum64())))
}

// generate a random identifier name
func (g *idgen) ident() string {
	bts := make([]byte, idxLen)
	for i := range bts {
		bts[i] = idxChars[g.rand.Intn(len(idxChars))]
	}
...
func (m *Map) SetVarname(s string) {
	...
	idgen := newIdgen("map", s)
ridx:
	m.Keyidx = idgen.ident()
	m.Validx = idgen.ident()
...
func (s *Slice) SetVarname(a string) {
	...
	s.Index = newIdgen("slice", s.TypeName(), s.Varname()).ident()
	varName := s.Varname()
	if varName[0] == '*' {
